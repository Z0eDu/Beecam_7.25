...
class Tag_detect(threading.Thread):
    ...
    def run(self):
        with lock:
            ...
            #pred=self.tag_read(tag)
            #print (pred)
                f.write(str(self.cnt)+'   '+str(pred)+"\n")
            else:
                f.write(str(self.cnt)+'   '+"None\n")
        ...
        while not self.terminated:
            ...
            if self.event.wait(1):
                try:
                    ...
                    #cv2.imwrite("/home/pi/Desktop/4/"+str(self.cnt)+'.png',np.array(im.convert('RGB') ))
                    if flag:
                        q.put((origin,self.cnt))
                        threads.append(Tag_detect(q,self.cnt).start())
                finally:
...
pool = [ImageProcessor() for i in range(25)]
